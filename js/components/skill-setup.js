...
export function skillSetup(guide) {
  ...
  const special = (guide.specialSkillGroups || []).map(group => `<h3 class="group-label">${escape(group.title)}</h3><div class="skill-grid">${group.skills.map(skillCard).join('')}</div>`).join('');
