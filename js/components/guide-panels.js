...
export function gemsPanel(guide) {
  ...
  return `<div class="gem-summary">${badge(`${gems.damage.length} Damage`, 'pink')}${badge(`${gems.cooldown.length} Cooldown`, 'purple')}${gems.filler ? badge('1 Filler') : ''}</div>${gems.referenceLabel ? `<p class="notice">${escape(gems.referenceLabel)}</p>` : ''}${gems.emphasis ? infoPanel(gems.emphasis) : ''}<h3>Damage Gems</h3>${gemTable(gems.damage, [...guide.skills, ...(guide.demonSkills || [])], gems.referenceLevels?.damage || gems.damagePriorities || {}, 'Damage', guide.gemTargets, reference, 'Prioridade', gems.damageRanks)}<p>${escape(gems.damageNote)}</p><h3>${reference || gems.cooldownRankLabel ? 'Cooldown Gems' : 'Cooldown Gems · Upgrade Priority'}</h3>${gems.practical ? `<aside class="notice"><strong>Minimum practical</strong><p>${escape(gems.practical)}</p></aside>` : ''}${gemTable(gems.cooldown, [...guide.skills, ...(guide.demonSkills || [])], gems.referenceLevels?.cooldown || gems.recommended, 'Cooldown', guide.gemTargets, reference, gems.cooldownRankLabel, gems.cooldownRanks)}${(gems.upgradeNotes || []).map((note) => `<p>${escape(note)}</p>`).join('')}${gems.filler ? `<div class="panel filler"><span class="gem-diamond" aria-hidden="true">◇</span><div><h3>11th Gem — Filler</h3><p>${escape(gems.filler)}</p></div></div>` : ''}`;
}
