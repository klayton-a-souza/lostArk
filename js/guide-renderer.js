import { skillSetup } from './components/skill-setup.js';
...
export function renderGuide(guide) {
  ...
  ${section('skills', '04', 'Skill Setup', `${guide.skills.length} skills · Tripods, runas e função no ciclo.`, `${guide.skillSetupLabel ? `<h3>${escape(guide.skillSetupLabel)}</h3><p>${escape(guide.skillSetupNote)}</p>` : ''}${skillSetup(guide)}${importCodes(guide.skillImport)}${skillSnapshot(guide.skillSnapshot, guide.skills)}`)}
